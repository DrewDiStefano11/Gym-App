async function loadFitCoreApp() {
  ...
  source = source.replace("    state.aiMessages.push(normalizeAiMessage({ role: \"assistant\", content: localCoachReply(prompt), contextMode: aiContextMode, section: state.activeMode, errorState: error.message || \"AI unavailable\" }));", catchBlock);
  source = source.replace("function aiSuggestionRows() {", `${assistantUi}function aiSuggestionRows() {`);
  source = source.replace("      if (action === \"set-ai-context\") { aiContextMode = button.dataset.mode || \"quick\"; render(); }", aiActions);
